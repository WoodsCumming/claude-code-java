 ...
 * Returns {@code isSearch=false, isRead=false} for tools that should not
 * collapse (e.g., send_message, create_*, update_*).
 *
 * <p>Uses explicit per-tool allowlists for the most common MCP servers.
 ...
 * Unknown tool names do not collapse (conservative).</p>
 *
 * Translated from src/tools/MCPTool/classifyForCollapse.ts
 */
public final class McpCollapseClassifier {

    private McpCollapseClassifier() {}
    ...
    private static final Set<String> SEARCH_TOOLS = Set.of(
            ...
            "search_folders",
            // Supabase
            // "search_docs" already listed above
            // Stripe
            ...
            // "find_projects" already listed above
            "find_sections",
            "find_comments",
    ...
    /**
     * Classification result for an MCP tool.
     *
     * @param isSearch true if the tool is a search operation
     * @param isRead   true if the tool is a read/list operation
     */
    public record CollapseClassification(boolean isSearch, boolean isRead) {}

    /**
     * Classifies an MCP tool by server name and tool name.
     *
     * <p>The {@code serverName} parameter is accepted for API parity with the
     * TypeScript original but is currently unused — matching is keyed on the
     * normalised tool name only.</p>
     *
     * @param serverName ignored (present for API compatibility)
     * @param toolName   the MCP tool name (camelCase, kebab-case, or snake_case)
     * @return classification indicating whether the tool is search/read
     */
    public static CollapseClassification classifyMcpToolForCollapse(String serverName, String toolName) {
    ...
    /**
     * Normalizes a tool name: camelCase and kebab-case are converted to snake_case,
     * then lower-cased.
     */
    static String normalize(String name) {
        return name
                // Insert underscore between a lowercase letter and an uppercase letter (camelCase)
                .replaceAll("([a-z])([A-Z])", "$1_$2")
                // Replace hyphens with underscores (kebab-case)
                .replace('-', '_')
                .toLowerCase();
    }
}
