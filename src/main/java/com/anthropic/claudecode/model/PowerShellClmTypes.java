 ...
 *
 * Translated from src/tools/PowerShellTool/clmTypes.ts
 */
public final class PowerShellClmTypes {

    /**
     * The set of CLM-allowed type names (all lowercase).
     *
     * SECURITY notes:
     * - 'adsi' and 'adsisearcher' REMOVED — perform network LDAP binds on cast.
     * - 'cimsession' REMOVED — creates a CIM session (network connection).
     * - 'wmi', 'wmiclass', 'wmisearcher' REMOVED — WMI queries / remote access.
     * - DirectoryEntry / DirectorySearcher / ManagementObject FQ names REMOVED.
     */
    public static final Set<String> CLM_ALLOWED_TYPES = Set.of(
            ...
            "cimconverter",
            "ciminstance",
            // 'cimsession' REMOVED — see security note above
            "cimtype",
            "cmdletbinding",
            "cultureinfo",
            "datetime",
            ...
            // ManagementClass/ManagementObjectSearcher FQ REMOVED
            "system.collections.specialized.ordereddictionary",
            "system.security.accesscontrol.objectsecurity",
            // Arrays of allowed types (base name stored; normalizeTypeName strips [])
            "object",
            "system.object",
            // ModuleSpecification — fully qualified name
    ...
    public static String normalizeTypeName(String name) {
        ...
        return name.toLowerCase()
                .replaceAll("\\[\\]$", "")        // strip array suffix: String[] → string
                .replaceAll("\\[.*\\]$", "")       // strip generic args: List[int] → list
                .trim();
    }

    /**
     * Returns {@code true} if {@code typeName} (from AST) is in Microsoft's CLM allowlist.
     * Types NOT in this set trigger ask — they access system APIs CLM blocks.
     */
    public static boolean isClmAllowedType(String typeName) {
        return CLM_ALLOWED_TYPES.contains(normalizeTypeName(typeName));
    }

    private PowerShellClmTypes() {}
}
