...
public class RememberSkill {
    ...
    public static final String WHEN_TO_USE =
            "Use when the user wants to review, organize, or promote their auto-memory entries. "
                    + "Also useful for cleaning up outdated or conflicting entries across CLAUDE.md, CLAUDE.local.md, and auto-memory.";

    /**
     * Returns true when the skill is available (requires auto-memory to be enabled
     * and the USER_TYPE environment variable set to "ant").
     */
    ...
    /**
     * Builds the prompt content for the /remember command.
     *
     * @param args optional additional context supplied by the user
     * @return a list of prompt parts (each carrying a text block)
     */
    public CompletableFuture<List<PromptPart>> getPromptForCommand(String args) {
        return CompletableFuture.supplyAsync(() -> {
            StringBuilder prompt = new StringBuilder(SKILL_PROMPT);
    ...
    /**
     * Simple record representing a single prompt part (mirrors the TS
     * {@code { type: string, text: string }} shape used in getPromptForCommand).
     */
